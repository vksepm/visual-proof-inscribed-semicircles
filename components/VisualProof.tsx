...
const SQUARE_CENTER = { x: 0.5, y: 0.5 };
...
export const VisualProof: React.FC = () => {
  ...
  const steps = [
    {
      ...
      content: (
        <div className="space-y-4 text-base">
          ...
          <p className="bg-slate-100 p-3 rounded border border-slate-300 font-serif text-center">
    ...
    },
    {
      ...
      content: (
        <div className="space-y-4">
          ...
          <div className="font-mono bg-slate-100 p-2 rounded text-sm">
            Leg 1 = 1 - r<br/>
            Leg 2 = r<br/>
            Hypotenuse = 2r
          </div>
    ...
    },
    {
      ...
      content: (
        <div className="space-y-4">
          ...
          <p className="text-sm text-slate-600">We keep the positive root.</p>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      content: (
        <div className="space-y-4">
          ...
          <p className="font-mono text-xs sm:text-sm bg-slate-50 p-1">Width = 1/2 - (√3 - 1)/2 = (2 - √3)/2</p>
    ...
    },
    {
      ...
      content: (
        <div className="space-y-3">
          ...
          <div className="font-mono text-xs sm:text-sm bg-slate-100 p-2 rounded overflow-x-auto">
            D² = [(2-√3)/2]² + [1/2]²<br/>
            D² = (7-4√3)/4 + 1/4<br/>
            D² = (8-4√3)/4<br/>
            D² = 2 - √3
    ...
    },
    {
      ...
      content: (
        <div className="space-y-3">
          ...
          <div className="font-mono text-xs sm:text-sm bg-slate-50 p-2 rounded border-l-4 border-indigo-400">
             D = √[(4 - 2√3) / 2]<br/>
             <span className="text-slate-500 text-xs">// Numerator is a perfect square: (√3 - 1)²</span><br/>
             D = √[(√3 - 1)² / 2]<br/>
             D = (√3 - 1) / √2
          </div>
          <p className="text-sm">
      ...
      content: (
        <div className="space-y-4">
          ...
          <ul className="grid grid-cols-4 gap-2 text-center font-mono text-sm">
            <li className="bg-slate-100 p-1 rounded">a = 3</li>
            <li className="bg-slate-100 p-1 rounded">b = -1</li>
            <li className="bg-slate-100 p-1 rounded">c = 2</li>
            <li className="bg-slate-100 p-1 rounded">d = -1</li>
          </ul>
          <div className="mt-4 p-3 bg-emerald-100 text-emerald-800 rounded text-center font-bold">
            Sum = 3 + (-1) + 2 + (-1) = 3
          </div>
        </div>
      ),
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans text-slate-800 flex flex-col items-center w-full">
      
      <div className="max-w-4xl w-full mb-8 text-center">
        <p className="text-slate-500 text-lg">Inscribed Semicircles & The Small Circle</p>
      </div>

      <div className="max-w-5xl w-full grid grid-cols-1 md:grid-cols-2 gap-8 bg-white p-6 rounded-2xl shadow-xl border border-slate-200">
        
        {/* Left Column: Interactive Diagram */}
        <div className="relative aspect-square bg-slate-50 rounded-xl border border-slate-200 overflow-hidden select-none">
          ...
          <svg viewBox="-0.1 -0.1 1.2 1.2" className="w-full h-full">
            ...
            {(step >= 5) && (
               <g>
                 <circle cx={SQUARE_CENTER.x} cy={SQUARE_CENTER.y} r="0.015" fill="#ef4444" />
                 <circle cx={CENTERS[0].x} cy={CENTERS[0].y} r="0.015" fill="#1e40af" />
                 
                 {/* Line connecting Square Center to Bottom Semicircle Center */}
                 <line x1={SQUARE_CENTER.x} y1={SQUARE_CENTER.y} x2={CENTERS[0].x} y2={CENTERS[0].y} stroke="#f97316" strokeWidth="0.008" />
                 
                 {step >= 6 && (
                    <>
                      {/* Triangle Legs for Distance Calc */}
                      <line x1={SQUARE_CENTER.x} y1={SQUARE_CENTER.y} x2={SQUARE_CENTER.x} y2={CENTERS[0].y} stroke="#94a3b8" strokeWidth="0.005" strokeDasharray="0.02" />
                      <line x1={SQUARE_CENTER.x} y1={CENTERS[0].y} x2={CENTERS[0].x} y2={CENTERS[0].y} stroke="#94a3b8" strokeWidth="0.005" strokeDasharray="0.02" />
                      
                      <text x={SQUARE_CENTER.x + 0.02} y={(SQUARE_CENTER.y + CENTERS[0].y)/2} fontSize="0.04" fill="#f97316">0.5</text>
                      <text x={(SQUARE_CENTER.x + CENTERS[0].x)/2} y={CENTERS[0].y - 0.02} fontSize="0.04" fill="#f97316">|0.5 - r|</text>
                    </>
                 )}
               </g>
            )}
          ...
          {/* Vertex Labels */}
          <div className="absolute top-2 left-2 text-xs text-slate-400">(0,1)</div>
          <div className="absolute top-2 right-2 text-xs text-slate-400">(1,1)</div>
          <div className="absolute bottom-2 left-2 text-xs text-slate-400">(0,0)</div>
          <div className="absolute bottom-2 right-2 text-xs text-slate-400">(1,0)</div>
          ...
          <div className="mt-8 flex gap-4">
            <button 
              ...
              className={`flex-1 py-3 px-4 rounded-lg flex items-center justify-center gap-2 font-semibold transition-colors
                ${step === 0 ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-white border-2 border-indigo-600 text-indigo-600 hover:bg-indigo-50'}`}
            >
            ...
            <button 
              ...
              className={`flex-1 py-3 px-4 rounded-lg flex items-center justify-center gap-2 font-semibold transition-colors
                ${step === steps.length - 1 ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-lg shadow-indigo-200'}`}
            >
